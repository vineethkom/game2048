    ...
    public void tilt(Side side) {
        ...
        for (int c = 0; c < _board.size(); c+=1) {
            ...
            for (int r=2; r >= 0; r--) {
                ...
                int i = 0;
                while (r + 1 + i <=  3) {
                    if (_board.tile(c, r + 1 + i) == null) {
                        _board.move(c, r + 1 + i, t);
                        t = _board.tile(c, r + 1 + i);
                        //addTile(Tile.create(0, c, r + i));
                        i++;
                    }
                    else if (_board.tile(c, r + 1 + i).value() == t.value() &&
                            t.value() != lastMerge){
                        lastMerge = t.value() * 2;
                        //This means we are merging
                        _score += t.value()*2;
                        _board.move(c, r + 1 + i, t);
                        break;
                    }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for (int r = 0; r <= b.size()-1; r += 1) {
            for (int c = 0; c <= b.size()-1; c += 1) {
                if ((c+1 <= b.size()-1) && (b.tile(r, c).value() == b.tile(r, c + 1).value())){
                    return true;
                } else if ((r+1 <= b.size()-1) && (b.tile(r, c).value() == b.tile(r + 1, c).value())) {
                    return true;
                }
            }
        }
